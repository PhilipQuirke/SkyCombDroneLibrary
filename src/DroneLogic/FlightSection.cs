...
namespace SkyCombDrone.DroneLogic
{
    ...
    public class FlightSection : FlightSectionModel
    {
        ...
        public void CalculateSettings_LocationM(CountryLocation minCountryLocation, FlightSections sections)
        {
    ...
    public class FlightSections : FlightSectionsModel
    {
        ...
        // Relative origin is bottom left of box.
        // Absolute origin is FlightSections.MinGlobalLocation.Latitude/Longitude
        public DroneLocation GlobalToDroneLocation(CountryLocation minCountryLocation, GlobalLocation thisGlobalLocation)
        {
            if (!thisGlobalLocation.Specified)
                return new(0, 0);

            var thisCountryLocation = ConvertDJILocationHeight.Wgs84ToNztm(thisGlobalLocation);

            return new(thisCountryLocation.NorthingM - minCountryLocation.NorthingM, thisCountryLocation.EastingM - minCountryLocation.EastingM);
        }


        // Calculate global location of the specified relative object location. 
        public GlobalLocation DroneToGlobalLocation(DroneLocation objectLocation)
        {
            // Convert object location from DroneLocation (0..3000x0..3000) to CountryLocation (NZTM2000)
            CountryLocation objectCountryLocation = new(
                MinCountryLocation.NorthingM + objectLocation.NorthingM,
                MinCountryLocation.EastingM + objectLocation.EastingM);

            return ConvertDJILocationHeight.NztmToWgs84(objectCountryLocation);
        }
        ...
        public void CalculateSettings()
        {
            ...
            {
                thisSection.CalculateSettings_LocationM(MinCountryLocation,this);
